...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  const imageUrl = product.images && product.images.length > 0
    ? product.images[0]
    : `https://ui-avatars.com/api/?name=${encodeURIComponent(product.name)}&size=200&background=DBA39A&color=ffffff&bold=true`;
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {product.desc && (
          <p className="text-xs text-[#3d2c28]/60 mt-1 line-clamp-2">{product.desc}</p>
        )}
        <div className="flex items-center gap-2 mt-2">
          <span className="text-lg font-bold text-[#DBA39A]">
            Ksh.{(product.offerPrice || product.price).toFixed(2)}
          </span>
          {product.offerPrice && (
